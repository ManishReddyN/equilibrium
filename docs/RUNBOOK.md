...
- **Android release keystore**: `TODO(user)`: generate an upload keystore locally and record its location outside version control. Blocked as of 2026-07-19: this machine has no local JDK, and the Docker Desktop route (`keytool` via a JDK container) is currently broken -- `wsl -l -v` shows the `docker-desktop` WSL2 distro stuck `Stopped` even after killing and relaunching Docker Desktop.exe, so `docker info` hangs indefinitely rather than the engine ever coming up. This looks like a WSL2/virtualization config issue on this machine rather than something a retry fixes. Fix Docker Desktop (restart the machine, `wsl --update`, or reinstall) and I'll pick the keystore generation back up, or let me know if you'd rather generate it yourself/some other way. Once it exists: `keytool -genkeypair -v -storetype PKCS12 -keystore upload-keystore.jks -alias upload -keyalg RSA -keysize 2048 -validity 10000`, saved outside version control (`android/app/build.gradle`'s `release` signingConfig reads it via a gitignored `android/keystore.properties` -- see `android/keystore.properties.example` for the format -- for local builds; CI/Fastlane instead injects `ANDROID_KEYSTORE_PATH`/`_PASSWORD`/`ANDROID_KEY_ALIAS`/`_PASSWORD` as secrets, see `release-android.yml`).
