...
- **Supabase project**: reusing the existing `roomie` project ref `zubxuqshcyniosmdztmw` rather than creating a new one. ~~`TODO(user)`: confirm `SUPABASE_ACCESS_TOKEN` access to this project for CI~~ — resolved 2026-07-20: user ran `supabase login` locally and confirmed the old `roomie` web app is dead (safe to have dropped its legacy schema); all migrations through `0009` are now applied on production. CI's `deploy-backend.yml` still doesn't have `SUPABASE_ACCESS_TOKEN` as a *repo secret* though (that's a separate, deliberate step — see below) — this local CLI login only enables manual `supabase db push`/etc. from this machine.
